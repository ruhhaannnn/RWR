...
function switchTab(tabId, el) {
    ...
    setTimeout(() => {
        ...
        // Fetch new proxies in background smoothly when tab is opened
        if(tabId === 'proxy-section') fetchProxies();
    }, 300);
...
// --- BLAZING FAST CORS ENGINE (Parallel Processing) ---
async function fetchWithFastestProxy(targetUrl, type = 'json') {
    ...
    // Fire all CORS requests simultaneously and take the one that answers first
    const requests = [
        fetch(`https://api.allorigins.win/raw?url=${encodeURIComponent(freshUrl)}`, { cache: "no-store" }),
        fetch(`https://corsproxy.io/?url=${encodeURIComponent(freshUrl)}`, { cache: "no-store" }),
        fetch(`https://api.codetabs.com/v1/proxy?quest=${encodeURIComponent(freshUrl)}`, { cache: "no-store" })
    ];

    try {
        const res = await Promise.any(requests.map(p => p.then(r => { 
            if (!r.ok) throw new Error('Bad response'); 
            return r; 
        })));
        return type === 'json' ? await res.json() : await res.text();
    } catch(e) { throw new Error("CORS limits reached on all gateways."); }
}

setInterval(() => { document.getElementById('clock').innerText = new Date().toUTCString(); }, 1000);

// --- ORIGINAL OSINT SCRAPERS ---
...
function getStoredIntel() { 
    try {
        ...
        let cleanedData = data.filter(d => typeof d.timestamp === 'number' && !isNaN(d.timestamp));
        return cleanedData;
    } catch(e) {
...
function saveStoredIntel(dataArray) { localStorage.setItem('iqwr_intel_db', JSON.stringify(dataArray.slice(-500))); }
...
async function fetchLiveOSINT() {
    try {
        const [amkData, rnintelData, ddData] = await Promise.all([
            scrapeTelegramChannel('AMK_Mapping', false), scrapeTelegramChannel('rnintel', false), scrapeTelegramChannel('DDGeopolitics', false)
        ]);
        const posts = [...(amkData||[]), ...(rnintelData||[]), ...(ddData||[])];
...
function renderMapData() {
    ...
    filtered.forEach(strike => {
        ...
        feedElement.insertAdjacentHTML('beforeend', `
            <div class="feed-entry ${strike.eventType}" onclick="flyToLoc(${strike.lng}, ${strike.lat}, '${strike.id}')">
        ...
        const popupHTML = `<div style="display:flex; justify-content:space-between; margin-bottom: 6px; border-bottom: 1px solid #333; padding-bottom: 4px;">
            ...
            <div style="font-size:0.7em; color:#888; text-transform:uppercase; border-top: 1px dashed #333; padding-top: 4px;">SOURCE: ${strike.source}</div>`;
...
// ==========================================
// REAL-TIME PROXY ENGINE (BASIC, PRO, EMERGENCY)
// ==========================================

async function fetchProxies() {
    
    // 1. BASIC: MTProto (Multiple Sources & Improved Link Detection)
    const basicSources = [
    ...
    Promise.any(basicSources.map(url => fetchWithFastestProxy(url, 'text'))).then(rawText => {
        let mtHtml = '';
        let validCount = 0;
        
        rawText.split('\n').forEach(line => {
            // Eradication Logic: Must contain actual proxy parameters
            if(line.includes('server=') && line.includes('secret=') && validCount < 6) {
                
                // Auto-convert tg:// to https://t.me/ for web clicking
                let cleanLink = line.trim();
                if(cleanLink.startsWith('tg://')) {
                    cleanLink = cleanLink.replace('tg://', 'https://t.me/');
                }

                mtHtml += `<a href="${cleanLink}" target="_blank" style="display:block; padding:12px; background:#1e293b; color:#3b82f6; text-decoration:none; border-radius:4px; text-align:center; font-size:0.8rem; border:1px solid #334155; font-weight:bold; font-family:var(--font-mono); margin-bottom:8px; transition:0.2s;" onmouseover="this.style.background='#0f172a'" onmouseout="this.style.background='#1e293b'">⚡ CONNECT MTPROTO</a>`;
                validCount++;
            }
        });
        
        if(mtHtml) {
            document.getElementById('proxy-basic-feed').innerHTML = mtHtml;
            saveToCache('proxy-basic-feed', mtHtml);
        } else {
            document.getElementById('proxy-basic-feed').innerHTML = '<div style="text-align:center; color:#ef4444;">No MTProto relays found.</div>';
        }
    }).catch(() => console.warn("Basic fetch delayed or blocked"));

    // 2. PRO: GitHub V2Ray/VLESS (Instant Parallel Fetch)
    const proSources = [
        "https://raw.githubusercontent.com/barry-far/V2ray-Config/main/Splitted-By-Protocol/vless.txt",
        "https://raw.githubusercontent.com/yebekhe/TelegramV2rayCollector/main/sub/normal/vless"
    ];
    
    Promise.any(proSources.map(url => fetchWithFastestProxy(url, 'text'))).then(rawText => {
        let proHtml = '';
        let validPro = 0;
        // Eradication Logic: Structural validation, max 6 items
        rawText.split('\n').filter(line => line.includes('vless://') && line.length > 80).forEach(line => {
            if(validPro < 6) {
                proHtml += `<div onclick="copyProxyText(this)" style="background:#000; color:#22c55e; padding:10px; border-radius:4px; font-size:0.7rem; word-break:break-all; border:1px solid #333; cursor:pointer; font-family:var(--font-mono); max-height:80px; overflow-y:auto; transition:0.2s; margin-bottom:8px;" onmouseover="this.style.borderColor='#22c55e'" onmouseout="this.style.borderColor='#333'">${line.trim()}</div>`;
                validPro++;
            }
        });
        if(proHtml) {
            document.getElementById('proxy-pro-feed').innerHTML = proHtml;
            saveToCache('proxy-pro-feed', proHtml);
        }
    }).catch(() => console.warn("Pro fetch delayed"));

    // 3. EMERGENCY: Fallback (VMess/SS Parallel Fetch)
    const emSources = [
        "https://raw.githubusercontent.com/mahdibland/V2RayAggregator/master/sub/sub_merge.txt",
        "https://raw.githubusercontent.com/Epodonios/v2ray-configs/main/All_Configs_Sub.txt"
    ];

    Promise.any(emSources.map(url => fetchWithFastestProxy(url, 'text'))).then(rawText => {
        let emHtml = '';
        let validEm = 0;
        // Eradication Logic: Filter for non-VLESS protocols, max 6 items
        rawText.split('\n').filter(line => (line.includes('vmess://') || line.includes('ss://')) && line.length > 50).forEach(line => {
            if(validEm < 6) {
                let type = line.split('://')[0].toUpperCase();
                emHtml += `<div onclick="copyProxyText(this)" style="background:#1a1a1a; color:#f59e0b; padding:10px; border-radius:4px; font-size:0.7rem; word-break:break-all; border:1px solid #451a03; cursor:pointer; font-family:var(--font-mono); max-height:80px; overflow-y:auto; transition:0.2s; margin-bottom:8px;" onmouseover="this.style.borderColor='#f59e0b'" onmouseout="this.style.borderColor='#451a03'">
                    <div style="font-weight:bold; margin-bottom:4px; font-size:0.6rem;">[${type} FALLBACK]</div>${line.trim()}
                </div>`;
                validEm++;
            }
        });
        if(emHtml) {
            document.getElementById('proxy-emergency-feed').innerHTML = emHtml;
            saveToCache('proxy-emergency-feed', emHtml);
        }
    }).catch(() => console.warn("Emergency fetch delayed"));
}

// Copy Action Feedback
function copyProxyText(element) {
    navigator.clipboard.writeText(element.innerText.replace(/\[.*?\]\n?/, '').trim()); // Removes the [FALLBACK] tag
...
window.onload = () => {
    ...
    // Proxy Caches (Instant Loading on Tab Switch)
    loadFromCache('proxy-basic-feed');
    loadFromCache('proxy-pro-feed');
    loadFromCache('proxy-emergency-feed');
};
...
map.on('load', () => {
    ...
    // Initial parallel fetch of all data
    Promise.allSettled([ fetchTicker(), fetchSummary(), fetchNews(), fetchIranNews(), fetchAirspaceStatus(), fetchLiveOSINT(), fetchProxies() ]);
    ...
    // Auto-refresh Proxies every 30 minutes to eradicate dead links
